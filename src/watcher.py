...
from file_ops import GitignoreManager, PathUtils
...
class MemoryProxyWatcher:
    ...
    def start(self) -> bool:
        ...
        if not watch_dirs:
            logger.error("No valid directories specified in AGENT_MEMORY_PATHS. Stopping execution...")
            sys.exit(1)
        ...
        if not all_configs:
            logger.error("No direcrories with configuration file were found. Stopping execution...")
            sys.exit(1)
    ...
    def _add_watcher(self, config_path: Path) -> None:
        try:
            ...
            self._observer.schedule(handler, str(watch_path), recursive=config.recursive)
