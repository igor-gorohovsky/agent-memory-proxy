...
class FileMatcher:
    ...
    def find_sync_targets(self, modified_file: Path) -> List[Tuple[Path, Path]]:
        ...
        for target, source in self.config.mappings.items():
            ...
            elif (self.config.recursive and 
                  modified_file.name == source_path.name and 
                  modified_file.is_relative_to(self.config.directory)):
...
class MemorySyncHandler(FileSystemEventHandler):
    ...
    def _should_process_file(self, file_path: Path) -> bool:
        ...
        if self.debouncer.should_debounce():
            logger.debug(f"Debouncing {file_path} - currently syncing or too recent")
            return False
    ...
    def _sync_all_targets(self, targets: List[Tuple[Path, Path]]) -> List[Path]:
    ...
    def _log_sync_results(self, source_file: Path, synced_paths: List[Path]) -> None:
        """Log the results of sync operations"""
        if not synced_paths:
            return
            
        source_dir, source_name = PathUtils.get_relative_path_info(source_file, self.config.directory)
