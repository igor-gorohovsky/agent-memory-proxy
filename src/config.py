    ...
    def validate_agents_list(agents) -> list[str]:
        ...
        for agent in agents:
            if not isinstance(agent, str):
                raise ValueError(
                    f"Agent name must be string, got {type(agent)}: {agent}"
                )
...
class MemoryProxyConfig:
    ...
    def _load_and_validate_config(self) -> None:
        ...
        try:
            ...
            self.truth_memory_file = config_data.get(
                "truth_memory_file", "AGENT.md"
            )

            # Validate and process agents
            agents = ConfigValidator.validate_agents_list(
                config_data["agents"]
            )
            self.mappings = ConfigValidator.create_mappings(
                agents, self.truth_memory_file
            )
